...
PUBLIC_METHODS = {
    ...
    "/usuarios/login": ["POST","OPTIONS"],  # Permitir login sin autenticación
    "/productos": ["GET"],  # Permitir ver catálogo de productos sin autenticación
    "/productos/": ["GET"],  # Permitir también con barra final
}
...
def create_access_token(data: dict[str, Any], expires_delta: Optional[timedelta] = None) -> str:
    ...
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=os.getenv('ALGORITHM', 'HS256'))  # type: ignore
    return encoded_jwt

def verify_jwt_token(token: str) -> dict[str, Any]:
    """Verificar y decodificar un token JWT"""
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[os.getenv('ALGORITHM', 'HS256')])  # type: ignore
        return payload
