...
from .dto import CartItemCreate, CartItemOut, CartItemUpdate
...
@cart_items.get('/user/{user_id}', response_model=List[CartItemOut], status_code=status.HTTP_200_OK)
def get_user_cart(user_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error interno del servidor al obtener el carrito del usuario"
        )
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error inesperado al obtener el carrito del usuario"
        )
